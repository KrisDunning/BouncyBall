...
var score=0;var lives=3;v
ar x=canvas.width/2;var y=canvas.height-30;
...
function draw(){
  ...
  if(y+dy<ballRadius){
    ...
    if(x>paddleX && x<paddleX+paddleWidth){
      dy=-dy;
      paddleColor='#'+ Math.floor(Math.random()*16777215).toString(16);            // possible to use left/right side of paddle to change angle            x<paddleX+paddleWidth/2?dx-=.2:dx+=.2;        }        else{            lives--;            if(!lives){                alert("Game Over!");                document.location.reload();            }            else{                x=canvas.width/2;                y-canvas.height-30;                dx=2;                dy=-2;                paddleX=(canvas.width-paddleWidth)/2;            }        }    }    if(rightPressed && paddleX<canvas.width-paddleWidth){        paddleX+=7;    }    if(leftPressed && paddleX>0){        paddleX-=7;    }    x+=dx;    y+=dy;    requestAnimationFrame(draw);}
function drawBall(){
  ctx.beginPath();
